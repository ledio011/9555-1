...
def inspect_sproto(data, offset=0):
    if len(data) < 4:
        return None
    try:
        field_count = struct.unpack_from("<H", data, offset)[0]
        header_size = offset + 2 + field_count * 2
        ...
        return {"field_count": field_count, "fields": fields}
    except Exception:
        return None
...
def inspect_frame(frame):
    ...
    for tag, value in info["fields"].items():
        if isinstance(value, int):
            log(f"SPROTO TAG {tag}: INT = {value}")
        else:
            log(f"SPROTO TAG {tag}: {len(value)} bytes")
            if len(value) <= 64:
                log(f"  DATA: {hexdump(value, 64)}")
    ...
    try:
        while True:
            ...
            if header is None:
                ...
                break
            frame_size = struct.unpack(">H", header)[0]
            frame_number += 1
            log(f"[RX] FRAME #{frame_number} SIZE={frame_size}")
            if frame_size == 0:
                log("[RX] Empty frame")
                continue
            frame = recv_exact(conn, frame_size)
            if frame is None:
                log("[DISCONNECT] while receiving frame")
                break
            inspect_frame(frame)
            log(f"[WAIT] Frame #{frame_number} recorded. No fabricated response sent.")
