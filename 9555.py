    ...
    try:
        ...
        while True:
            ...
            if msg == 4: # login
                ...
                send_rpc_push(668, encode_sproto([(0, 0), (1, 0), (2, {})]))
                if session is not None:
                    ph = encode_sproto([(1, session)]); pf = sproto_pack(ph + encode_sproto([]))
                    conn.sendall(struct.pack(">H", len(pf)) + pf)

            elif msg == 288: # request_sign_week_info
                send_rpc_push(661, encode_sproto([(0, 0), (1, 0), (2, {})]))
                if session is not None:
                    ph = encode_sproto([(1, session)]); pf = sproto_pack(ph + encode_sproto([]))
                    conn.sendall(struct.pack(">H", len(pf)) + pf)
                ...
                send_rpc_push(573, encode_sproto([(0, 0), (1, 0)]))
                if session is not None:
                    ph = encode_sproto([(1, session)]); pf = sproto_pack(ph + encode_sproto([]))
                    conn.sendall(struct.pack(">H", len(pf)) + pf)

            elif msg == 195: # request_slot_info
                send_rpc_push(568, encode_sproto([(0, 0), (1, 0)]))
                if session is not None:
                    ph = encode_sproto([(1, session)]); pf = sproto_pack(ph + encode_sproto([]))
                    conn.sendall(struct.pack(">H", len(pf)) + pf)
