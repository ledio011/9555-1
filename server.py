...
def sproto_pack(data):
    ...
    while i < n:
        ...
        if mask == 0xFF:
            # Look ahead for more 0xFF blocks
            ff_blocks = [chunk]
            j = i + 8
            while j < n and len(ff_blocks) < 256:
                next_chunk = data[j:j+8]
                if len(next_chunk) < 8: next_chunk += b'\x00' * (8 - len(next_chunk))
                if all(b != 0 for b in next_chunk):
                    ff_blocks.append(next_chunk)
                    j += 8
                else: break
            
            out.append(0xFF)
            out.append(len(ff_blocks) - 1)
            for b in ff_blocks: out.extend(b)
            i += len(ff_blocks) * 8
        else:
            out.append(mask)
            for j in range(8):
...
def send_push_rpc(conn, tag, data):
    ...
    pkg_h = encode_sproto([(0, tag), (1, server_session_counter)], fn=2)
    full = sproto_pack(pkg_h + data)
    conn.sendall(struct.pack(">H", len(full)) + full)
    print(f"[PUSH] Tag {tag} (Session: {server_session_counter})")
    ...
    try:
        while True:
            ...
            if msg_type == 4: # login
                ...
                print(f"[LIST] Sending for {acc_id}")
                if acc_id in characters:
                    c = characters[acc_id]
                    gen = encode_sproto([(0, c['name']), (1, c['prof']), (3, "101")], fn=4)
                    attr_ov = encode_sproto([(0, 1), (1, 55653)], fn=2)
                    defaults = {0: "XD_A_T", 1: "QJ_A_T", 2: "NQS_A_T"}
                    vis = encode_sproto([(0, c['name']), (1, "100"), (2, defaults.get(c['prof'], "XD_A_T"))], fn=3)
                    char_ov = encode_sproto([(0, c['id']), (1, gen), (2, attr_ov), (3, vis), (4, int(time.time()))], fn=6)
                    resp = encode_sproto([(0, [char_ov])], fn=1)
                else: resp = encode_sproto([(0, [])], fn=1)
                pkg_h = encode_sproto([(1, session)], fn=2); full = sproto_pack(pkg_h + resp)
                conn.sendall(struct.pack(">H", len(full)) + full)

            elif msg_type == 141: # sync_item_pack (Pre-pick requirement)
                print(f"[SYNC-INV] Unblocking UI for {acc_id}")
                resp = encode_sproto([(0, [])], fn=1) # Empty item list
                pkg_h = encode_sproto([(1, session)], fn=2); full = sproto_pack(pkg_h + resp)
                conn.sendall(struct.pack(">H", len(full)) + full)
                ...
                print(f"[PICK] Starting game for {acc_id}")
                ...
                print(f"[READY] Sourcing character data...")
                c = characters.get(acc_id)
                if c:
                    attr_full = encode_sproto([(0, 10560), (2, 100), (3, 50), (13, 800)], fn=25)
                    ...
                    vis = encode_sproto([(0, c['name']), (1, "100"), (2, "XD_A_T")], fn=3)
                    char_data = encode_sproto([(0, c['id']), (1, gen), (2, encode_sproto([(0, 10560), (3, 55653)], fn=19)), (6, vis), (7, mov), (13, runtime)], fn=17)
...
print(f"GAME SERVER READY ON {PORT} (X-RAY FINAL FIX)")
